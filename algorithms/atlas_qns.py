...
atlas_api_wrapper_url = " http://admin:admin@13.81.125.172:21000/api/atlas/v2/search/basic?query="
        ...
        for qns in qns_list:
            if qns.get("typeName") is not None:
                if typeName == qns["typeName"]:
                    param_details["pattern"] = qns["patterns"]
...
def get_qualified_name(typeName, input_params):
    ...
    for input in required_inputs["input_parameters"]:
        if input["name"] not in input_params.keys():
...
def create_qualified_name(typeName, input_params,required_inputs):
    ...
    for inputs in required_inputs["input_parameters"]:
        ...
        str(qualified_name).replace("{"+str(param_name)+"}",param_value)
...
def verify_qualified_name(typeName, qualifiedName):
    ...
    req = requests.get(url=str(atlas_api_wrapper_url) + query)
    if req.status_code == 200:
        ...
        for entity in data["entities"]:
            ...
            if e_typeName.get("typeName") == typeName and e_attributes("qualifiedName") == qualifiedName:
                qualifiedName["isExists"] = True
                qualifiedName["guid"] = entity.get("guid")
    else:
...
print(json.dumps(get_list_inputs("azure_sql_column_sg"))